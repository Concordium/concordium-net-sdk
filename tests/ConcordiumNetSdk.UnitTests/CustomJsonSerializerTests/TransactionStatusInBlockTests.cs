...
public class TransactionStatusInBlockTests
{
    // todo: find out about more json responses
    ...
    public async Task Should_correctly_deserialize_json_data()
    {
        ...
        var filePath = Path.Combine(currentDirectory, @"CustomJsonSerializerTests/Files/transaction-status-in-block.json");
        var expectedJson = await File.ReadAllTextAsync(filePath);

        // Act
        var obj = CustomJsonSerializer.Deserialize<TransactionStatusInBlock>(expectedJson);
