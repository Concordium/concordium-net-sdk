...
public class BirkParametersTests
{
    ...
    public async Task Should_correctly_deserialize_json_data()
    {
        ...
        var filePath = Path.Combine(currentDirectory, @"CustomJsonSerializerTests/Files/birk-parameters.json");
        var expectedJson = await File.ReadAllTextAsync(filePath);

        // Act
        var obj = CustomJsonSerializer.Deserialize<BirkParameters>(expectedJson);
