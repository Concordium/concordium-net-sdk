...
public class TransactionStatusTests
{
    ...
    public async Task Should_correctly_deserialize_json_data()
    {
        ...
        var filePath = Path.Combine(currentDirectory, @"CustomJsonSerializerTests/Files/transaction-status.json");
        var expectedJson = await File.ReadAllTextAsync(filePath);

        // Act
        var obj = CustomJsonSerializer.Deserialize<TransactionStatus>(expectedJson);
