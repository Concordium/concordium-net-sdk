...
public class NextAccountNonceTests
{
    ...
    public async Task Should_correctly_deserialize_json_data()
    {
        ...
        var filePath = Path.Combine(currentDirectory, @"CustomJsonSerializerTests/Files/next-account-nonce.json");
        var expectedJson = await File.ReadAllTextAsync(filePath);

        // Act
        var obj = CustomJsonSerializer.Deserialize<NextAccountNonce>(expectedJson);
