...
public class UpdateContractParametersSerializerTests
{
    [Fact]
    public void Should_correctly_serialize_contract_parameters_in_right_format()
    {
        ...
        var receiveFunctionName = "receiveFunctionName";
        var schemaModule = GetSchemaModule(contractName, receiveFunctionName);
        var userInput = GetUserInput();
        var expectedUpdateSerializedContractParameters = GetExpectedSerializedContractParameters();

        // Act
        var serializedUpdateContractParameters = UpdateContractParametersSerializer.Serialize(
            contractName,
            receiveFunctionName,
            userInput,
            schemaModule);
        ...
        // Assert
        serializedUpdateContractParameters.Should().BeEquivalentTo(expectedUpdateSerializedContractParameters);
    }

    private Module GetSchemaModule(string contractName = "contractName", string receiveFunctionName = "receiveFunctionName")
    {
        var contents = new[]
        {
        ...
        var receiveParamType = new StructType(fields);
        var contractSchema = new Contract(null, null, new Dictionary<string, Type>{{receiveFunctionName, receiveParamType}});
        var contractSchemas = new Dictionary<string, Contract> { {contractName, contractSchema}};
