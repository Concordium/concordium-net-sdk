...
public class InitContractParametersSerializerTests
{
    [Fact]
    public void Should_correctly_serialize_contract_parameters_in_right_format()
    {
        ...
        var schemaModule = GetSchemaModule(contractName);
        var userInput = GetUserInput();
        var expectedInitSerializedContractParameters = GetExpectedSerializedContractParameters();

        // Act
        var serializedInitContractParameters = InitContractParametersSerializer.Serialize(
            contractName,
            userInput,
            schemaModule);

        // Assert
        serializedInitContractParameters.Should().BeEquivalentTo(expectedInitSerializedContractParameters);
    }

    private Module GetSchemaModule(string contractName = "contractName")
    {
        var contents = new[]
        {
        ...
        var initParamType = new StructType(fields);
        var contractSchema = new Contract(null, initParamType, new Dictionary<string, Type>());
