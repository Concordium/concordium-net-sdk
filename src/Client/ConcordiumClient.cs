...
public sealed class ConcordiumClient : IDisposable
{
    ...
    public async IAsyncEnumerable<ISpecialEvent> GetBlockSpecialEvents(IBlockHashInput blockHashInput)
    {
        await foreach(var specialEvent in this.Raw.GetBlockSpecialEvents(blockHashInput.Into()).ConfigureAwait(false))
        {
